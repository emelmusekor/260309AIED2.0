...
def mgraph(center: str, *branches: tuple[str, str]) -> dict[str, object]:
    return {'center': center, 'branches': [{'relation': relation, 'label': label} for relation, label in branches]}
...
UI = {
    'ko': {'menuLabel':'메뉴','previewLabel':'핵심 요약','pagesLabel':'페이지 원본 보기','pageModalTitle':'페이지 원본','keywordModalTitle':'키워드','closeLabel':'닫기','keywordExcerptLabel':'원문 근거','keywordUsedLabel':'원문 사용 위치','pagesFactLabel':'원본 범위','coverageFactLabel':'분량','browseLabel':'목차부터 보기','jumpLabel':'본문 바로 보기','infoLabel':'보고서 정보','graphLabel':'개념 지도','searchPlaceholder':'검색어로 장, 카드, 해시태그 찾기','searchClearLabel':'초기화','searchDefaultLabel':'전체 내용을 보고 있습니다.','searchEmptyLabel':'일치하는 결과가 없습니다.','searchResultsPattern':'{count}개 결과','fallback':'이 구간은 원문 페이지에서 바로 확인할 수 있습니다.'},
    'en': {'menuLabel':'Menu','previewLabel':'Core Summary','pagesLabel':'Open Source Pages','pageModalTitle':'Source Page','keywordModalTitle':'Keyword','closeLabel':'Close','keywordExcerptLabel':'Source Evidence','keywordUsedLabel':'Used in source','pagesFactLabel':'Source pages','coverageFactLabel':'Coverage','browseLabel':'Browse sections','jumpLabel':'Jump to content','infoLabel':'Report info','graphLabel':'Concept map','searchPlaceholder':'Search sections, cards, and hashtags','searchClearLabel':'Clear','searchDefaultLabel':'Showing the full report.','searchEmptyLabel':'No matching results.','searchResultsPattern':'{count} results','fallback':'Open the source pages for the original wording and layout.'},
}
...
MAIN_KO = [
    {'id':'intro','nav':'소개','items':[('문제 제기',3,3),('연구 서문',4,4),('목차',5,5),('AI교육 2.0 소개',6,6),("왜 'AI교육 2.0'인가?",7,8),('AI교육 2.0의 목표와 대상',9,10)]},
    {'id':'principles','nav':'핵심 원칙','items':[('핵심 원칙 개요',11,11),('모형의 기반: 디지털로부터 출발',12,12),('AGI·ASI를 고려한 교육',13,13),('AI를 제대로 이해해야 하는 이유',14,14),('AI 시대를 준비하는 새로운 사고력',15,15),('AI와 협력해야 하는 이유',16,18),('윤리보다 가치교육',19,19),('새로운 학습이론의 필요성',20,20),('두려움보다 가능성에 주목하는 교육',21,22)]},
    ...
    {'id':'references','nav':'참고문헌','items':[('참고문헌 안내',62,62),('참고문헌',63,64)]},
]
MAIN_EN = [
    {'id':'introduction','nav':'Introduction','items':[('Research Introduction',2,2),('Index',3,3),("Why \"AI Education 2.0\"?",4,4),("Goals and Targets of \"AI Education 2.0\"",5,5)]},
...
SECONDARY_MAIN = [
    {'id':'intro','nav':'소개','items':[('머리말',3,4),('목차',5,6),('AI 협력의 첫걸음 도입',7,7)]},
    {'id':'first-steps','nav':'AI 협력의 첫걸음','items':[('AI 생성 시대와 교육의 변화',8,12),('AI협력 교육의 개념과 의의',13,17),('AI협력 교육 설계 원리',18,22)]},
...
SECONDARY_WORKBOOK = [
    {'id':'intro','nav':'소개','items':[('목차',3,3)]},
    {'id':'getting-started','nav':'생성형 AI 시작하기','items':[('좋은 프롬프트란?',4,8),('제로샷? 원샷? 퓨샷?',9,12),('이게 최선일까요?',13,18),('진짜일까, 가짜일까?',19,22),('어때요, 참 쉽죠?',23,28),('같은 내용, 다른 표현',29,32),('넌 이제 내 거야!',33,36),('한 번 더 생각하고 물어보자',37,40)]},
...
ELEMENTARY_MAIN = [
    {'id':'intro','nav':'소개','items':[('머리말',3,4),('목차',5,9),('AI 협력의 첫걸음 도입',10,10)]},
    {'id':'first-steps','nav':'AI 협력의 첫걸음','items':[('생성형 AI의 확산과 교육적 시사점',11,14),('AI를 파트너로 보는 새로운 학습 패러다임',15,16),('AI교육2.0과 AI협력 교육',17,20),('인간-AI 역할 분담의 교육적 가치',21,24),('사고가 오가는 수업의 구조',25,27),('교사의 준비 - 기술보다 태도',28,29)]},
...
ELEMENTARY_WORKBOOK = [
    {'id':'intro','nav':'소개','items':[('목차',3,3)]},
    {'id':'effective-questions','nav':'효과적으로 질문하기','items':[('AI에게 역할을 설정하자!',4,8),('AI의 글쓰기 형식을 선택하자!',9,10),('AI의 말투를 조정해 보자!',11,12),('나처럼 해봐요, 이렇게!',13,16),('AI가 그린 그림, 내가 따라 그린 그림',17,25),('최종 프롬프트 개선 게임',26,28)]},
...
REPORT_PATTERNS = {
    'secondary-main': [
        ...
        {'match': r'제로샷|원샷|퓨샷', 'summary':['예시를 얼마나 제공하느냐에 따라 AI 출력이 어떻게 달라지는지 비교합니다.'], 'tags':[mtag('예시 기반 질문','예시'), mtag('입력 전략','입력'), mtag('출력 비교','출력'), mtag('실험','실험')], 'graph':mgraph('입력 전략 비교', ('방식','제로샷'), ('확장','원샷'), ('정교화','퓨샷'))},
        ...
        {'match': r'한 번 더 생각하고 물어보자|토론해 보자', 'summary':['추가 질문과 대화를 통해 답변을 확장하고 사고를 깊게 만드는 활동입니다.'], 'tags':[mtag('추가 질문','질문'), mtag('대화 확장','대화'), mtag('사고 심화','사고'), mtag('반문','반문')], 'graph':mgraph('대화 확장', ('출발','추가 질문'), ('상호작용','대화 확장'), ('결과','사고 심화'))},
...
REPORTS = [
    ...
    {'key':'secondary-main','group':'secondary-main','main':'main-ko','out':REPORTS_DIR / 'secondary-main','pdf':SECONDARY_MAIN_PDF,'asset':'secondary-main','lang':'ko','html_lang':'ko','mark':'중등','brand':'생성형 AI, 교실 속 협력 파트너','subbrand':'중등편 본문','eyebrow':'중등편 본문','title':'생성형 AI,','subtitle':'교실 속 협력 파트너','strap':'중학교 교실을 위한 본문형 가이드','desc':'중등편 본문 전체를 목차 흐름대로 다시 묶었습니다.','meta':[('형태','인쇄용 본문 PDF'),('발행처','연두에디션'),('발행일','2026년 2월 28일')],'sections':SECONDARY_MAIN,'keys':[],'lang_switch':None,'pdf_label':'원문 PDF 열기','footer':'본문 전 페이지를 항목별 카드에 다시 배치해 필요한 수업 사례로 바로 이동할 수 있게 했습니다.'},
    {'key':'secondary-workbook','group':'secondary-workbook','main':'main-ko','out':REPORTS_DIR / 'secondary-workbook','pdf':SECONDARY_WORKBOOK_PDF,'asset':'secondary-workbook','lang':'ko','html_lang':'ko','mark':'중등W','brand':'생성형 AI, 교실 속 협력 파트너','subbrand':'중등편 워크북','eyebrow':'중등편 워크북','title':'활동지로 바로 보는','subtitle':'중등 AI 협력 수업','strap':'차시별 워크시트와 교과 활동 모음','desc':'중등편 워크북을 활동 단위로 다시 묶었습니다.','meta':[('형태','인쇄용 워크북 PDF'),('발행처','연두에디션'),('발행일','2026년 2월 28일')],'sections':SECONDARY_WORKBOOK,'keys':[],'lang_switch':None,'pdf_label':'원문 PDF 열기','footer':'워크북 전 페이지를 주제별로 다시 묶어 필요한 활동지만 빠르게 찾아볼 수 있게 했습니다.'},
    {'key':'elementary-main','group':'elementary-main','main':'main-ko','out':REPORTS_DIR / 'elementary-main','pdf':ELEMENTARY_MAIN_PDF,'asset':'elementary-main','lang':'ko','html_lang':'ko','mark':'초등','brand':'생성형 AI, 교실 속 협력 파트너','subbrand':'초등편 본문','eyebrow':'초등편 본문','title':'초등 교실을 위한','subtitle':'AI 협력 수업 설계','strap':'이론, 활동, 교과 사례, 학생용 워크북까지 한 권으로 정리','desc':'초등편 본문을 섹션별로 다시 잘라 바로 탐색할 수 있게 만들었습니다.','meta':[('형태','인쇄용 본문 PDF'),('발행처','연두에디션'),('발행일','2026년 2월 28일')],'sections':ELEMENTARY_MAIN,'keys':[],'lang_switch':None,'pdf_label':'원문 PDF 열기','footer':'본문과 뒤쪽 학생용 워크북까지 모두 포함해 전체 분량을 메뉴 기반으로 탐색할 수 있게 했습니다.'},
    {'key':'elementary-workbook','group':'elementary-workbook','main':'main-ko','out':REPORTS_DIR / 'elementary-workbook','pdf':ELEMENTARY_WORKBOOK_PDF,'asset':'elementary-workbook','lang':'ko','html_lang':'ko','mark':'초등W','brand':'생성형 AI, 교실 속 협력 파트너','subbrand':'초등편 워크북','eyebrow':'초등편 워크북','title':'활동으로 바로 쓰는','subtitle':'초등 AI 워크북','strap':'질문, 검토, 확장, 성찰 활동 묶음','desc':'초등편 워크북을 활동 흐름에 맞춰 다시 묶었습니다.','meta':[('형태','인쇄용 워크북 PDF'),('발행처','연두에디션'),('발행일','2026년 2월 28일')],'sections':ELEMENTARY_WORKBOOK,'keys':[],'lang_switch':None,'pdf_label':'원문 PDF 열기','footer':'워크북 전 페이지를 활동 단위로 다시 배치해 수업 준비와 현장 활용에 바로 쓸 수 있게 했습니다.'},
]
...
def polish_tag_label(label: str, lang: str) -> str:
    ...
    if lang == 'ko':
        for suffix, replacement in [('에서',''), ('에게',''), ('으로',''), ('와',''), ('과',''), ('의',''), ('은',''), ('는',''), ('이',''), ('가',''), ('을',''), ('를','')]:
...
def graph_terms(title: str, tags: list[str], lang: str) -> dict[str, object]:
    relations = ['Context', 'Core', 'Application'] if lang == 'en' else ['배경', '핵심', '적용']
    center = title if len(title) <= 30 else title[:29].rstrip() + '...'
    labels = [tag for tag in tags if term_key(tag) != term_key(title)]
    if not labels:
        labels = ['Core idea'] if lang == 'en' else ['핵심 개념']
    branches = []
    for idx, label in enumerate(labels[:3]):
        branches.append({'relation': relations[idx % len(relations)], 'label': label})
...
def section_data(spec: dict, sec: dict, texts: dict[int,str], prefix: str, lang: str, rgb: str, keyword_index: list[dict]) -> dict:
    ...
    for title, start, end in sec['items']:
        ...
        graph = manual.get('graph') or graph_terms(title, tags, lang)
        tag_refs = []
...
def info_html(spec: dict, count: int, info_dir: Path) -> str:
    ...
    switch_html = ''.join(f'<a href="{item["href"]}" class="{"active" if item["active"] else ""}">{item["label"]}</a>' for item in report_switches(spec, info_dir))
    lang_html = ''.join(f'<a href="{item["href"]}" class="{"active" if item["active"] else ""}">{item["label"]}</a>' for item in lang_switches(spec, info_dir))
